...
- The `<Link>` wrapping the button currently has no explicit width constraint, which can make the button stretch full-width on mobile and may introduce subtle alignment inconsistencies.

## Proposed change
- Add `w-fit` to the `<Link>` so the button only occupies its intrinsic width.
- Ensure the flex wrapper remains `justify-center` and `items-center` and does not stretch the child.
